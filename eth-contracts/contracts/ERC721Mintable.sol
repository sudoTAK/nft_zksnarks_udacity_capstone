...
contract Pausable is Ownable {
	...
	event Unpause(address unpauser);
...
contract ERC721 is Pausable, ERC165 {
	...
	function approve(address to, uint256 tokenId) public {
		...
		require(tokenOwner != to, "ERC721: caller address and to address is same");
		require(isApprovedForAll(tokenOwner, msg.sender) || tokenOwner == msg.sender, "ERC721: caller is not authorised");
	...
	function getApproved(uint256 tokenId) public view returns (address) {
		...
		require(_exists(tokenId), "ERC721: token id does not exists");
	...
	function _mint(address to, uint256 tokenId) internal {
		...
		require(_exists(tokenId) || to != address(0), "ERC721: token id already minted or invalid address to");
	...
	function _transferFrom(
		...
		require(ownerOf(tokenId) == from, "from address is not owner of token");
...
contract DigiHouseToken is ERC721Metadata("DigiHouseToken", "DHT", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
